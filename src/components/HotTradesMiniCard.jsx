...
export default function HotTradesMiniCard({ item, onEvaluate }) {
  ...
  return (
    ...
    >
      ...
      {/* Company name */}
      {item.company_name && (
        <p className="text-xs text-slate-500 truncate leading-tight">{item.company_name}</p>
      )}
      ...
      <div className="flex items-center gap-1.5 flex-wrap">
        ...
        {item.expected_move_pct != null && (
          <span className={`text-xs font-mono tabular-nums ${isBullish ? 'text-emerald-500' : 'text-rose-500'}`}>
      ...
      {onEvaluate && (
        <button
          ...
          className={`mt-auto w-full text-xs font-semibold rounded-lg px-2 py-1.5 border transition-colors ${
            isBullish
