...
const SESSIONS = US_ENABLED ? ['India', 'US'] : ['India'];
      ...
      <div className="flex items-center justify-between mb-3 flex-wrap gap-2">
        <div>
          <h2 className="text-sm font-semibold text-slate-100 flex items-center gap-2">
            ...
            {!loading && meta && (
              sessionActive ? (
                <span className="inline-flex items-center gap-1 text-[9px] font-bold px-1.5 py-0.5 rounded-full border bg-emerald-500/10 border-emerald-500/25 text-emerald-400">
                  <span className="w-1.5 h-1.5 rounded-full bg-emerald-400 animate-pulse" />
                  LIVE
                </span>
              ) : (
                <span className="inline-flex items-center gap-1 text-[9px] font-bold px-1.5 py-0.5 rounded-full border bg-amber-500/10 border-amber-500/25 text-amber-400">
      ...
      {loading ? (
        ...
        <div className="bg-slate-900 border border-slate-800 border-dashed rounded-xl py-8 flex flex-col items-center justify-center gap-2">
          ...
          <p className="text-xs text-slate-700">
            Auto-generated 1h before market open · or trigger from Pre-Market page.
          </p>
        </div>
      ) : (
